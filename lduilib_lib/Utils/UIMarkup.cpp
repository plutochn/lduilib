...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	...
	DWORD len = 0;
	LPBYTE pByte = NULL;
	CRenderEngine::LoadFileContent(pstrFilename, pByte, len);

	ret = LoadFromMem(pByte, len, encoding);
	delete[] pByte;
...
{
    // 属性只能用英文，所以这样处理没有问题
    while( *pstr != _T('\0') && (*pstr == _T('_') || *pstr == _T(':') || isalnum(*pstr)) ) ++pstr;
}
    ...
    while( *pstrText != _T('\0') && *pstrText != cEnd ) {
        ...
        if( *pstrText == _T(' ') ) {
            *pstrDest++ = *pstrText++;
			...
			// 原来是 LPTSTR pstrTemp = ::CharNext(pstrText);
			// pstrText指针没动
            LPTSTR pstrTemp = (pstrText+1);
...
bool CMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
	...
	strncpy(m_szErrorXML, pstrLocation != NULL ? pstrLocation : _T(""), sizeof(m_szErrorXML) - 1);
